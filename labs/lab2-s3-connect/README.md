...
Navigate to **DataEngine UI > Pipeline Management > Pipeline Management** and click into your pipeline, $USER-hello-world-pipeline (right click + `Go To Builder`).
...
> **Note:** S3 credentials are available in the labs overview page.
...
- Element triggers fire when an S3 object is created or deleted. No polling, no wasted invocations
- DataEngine wraps each event in a CloudEvent envelope containing the bucket name and object key
- The DataEngine UI is how you wire triggers to functions, monitor pipeline status, and inspect logs
