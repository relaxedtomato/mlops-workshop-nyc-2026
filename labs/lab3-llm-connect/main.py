...
def init(ctx):
    ...
    # TODO (Step 2): Call validate_config() from common.config_utils to check required
    ...
    # TODO (Step 3): Initialize the LLM client using LLMClient.
...
def handler(ctx, event):
    ...
    # TODO (Step 1): Parse the S3 event using parse_s3_event(event).
    #                Log the bucket and key. Return early if no records found.

    # TODO (Step 2): Fetch the file from S3 using ctx.s3_client.get_object().
    #                Decode the body content to utf-8.

    # TODO (Step 3): Call llm_summary(ctx, content), log the result, and return.


def llm_summary(ctx, content):
    # TODO (Step 3): Return ctx.llm_client.summarize(content).
    pass
